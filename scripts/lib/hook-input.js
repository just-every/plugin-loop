...
function isChildSession(input) {
  if (process.env.LOOP_CHILD !== "1") return false;
  if (!process.env.LOOP_CHILD_CWD || !input || typeof input.cwd !== "string") return false;
  return path.resolve(input.cwd) === path.resolve(process.env.LOOP_CHILD_CWD);
}
